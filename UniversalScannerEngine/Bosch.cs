...
{
    class Bosch : ScanEngine, IScanner
    {
        ...
        public ILogger<Bosch> Logger { get; }
        public Bosch(ILogger<Bosch> logger, IConfig config) : base(logger, config)
        {
            this.Logger = logger;
        ...
        public override void reciever(IPEndPoint from, byte[] data)
        {
            ...
            if (data.Length == typeof(BoschBinaryAnswer).StructLayoutAttribute.Size)
            {
                ...
                {
                   Logger.LogWarning( "Warning: Bosch.reciever(): Packet with wrong header.");
                    return;
                }
            ...
            else
            {
                ...
                {
                    Logger.LogWarning( String.Format("Warning: Bosch.reciever(): Invalid ipv4 format: {0}", deviceIPv4Str));
                    ip = from.Address;
                }
                ...
                {
                    Logger.LogWarning( String.Format("Warning: Bosch.reciever(): Invalid ipv6 format: {0}", deviceIPv6Str));
                }
            }
        }
