...
{
    class Panasonic : ScanEngine, IScanner
    {
        ...
        public ILogger<Panasonic> Logger { get; }
        public Panasonic(ILogger<Panasonic> logger, IConfig config) : base(logger, config)
        {
            this.Logger = logger;
        ...
        Dictionary<UInt16, byte[]> parsePacket(byte[] data)
        {
            ...
            while (i < data.Length-4)
            {
                ...
                {
                    Logger.LogWarning( "Warning: Panasonic.parsePacket(): packet overflow");
                    return result;
                }
        ...
        public override void reciever(IPEndPoint from, byte[] data)
        {
            ...
            if (data.Length <= 52)
            {
                Logger.LogWarning( String.Format("Warning: Panasonic.reciever(): Invalid packet size: {0}", data.Length));
                return;
            }
