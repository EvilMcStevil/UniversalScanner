...
{
    class Dahua2 : ScanEngine, IScanner
    {
        ...
        public ILogger<Dahua2> Logger { get; }
        public Dahua2(ILogger<Dahua2> logger, IConfig config) : base(logger, config)
        {
            this.Logger = logger;
        ...
        public override void reciever(IPEndPoint from, byte[] data)
        {
            ...
            {
                Logger.LogWarning( String.Format("Warning: Dahua2.reciever(): recieved invalid frame (headerSize={0}, expected {1})!", header.headerSize, headerSize));
                return;
            }
            packetSize = data.Length - headerSize;
            if (NetworkUtils.littleEndian32(header.packetSize1) != packetSize)
            {
                Logger.LogWarning( String.Format("Warning: Dahua2.reciever(): recieved invalid frame (packetSize={0}, expected {1})!", NetworkUtils.littleEndian32(header.packetSize1), packetSize));
                return;
            }
            ...
            if (method == "client.notifyDevInfo")
            {
                ...
                {
                    ip = from.Address;
                    Logger.LogWarning( String.Format("Warning: Dahua2.reciever(): Invalid ipv4 format: {0}", deviceIPv4));
                }
                ...
                if (deviceIPv6 != null)
                {
                    ...
                    {
                        Logger.LogWarning( String.Format("Warning: Dahua2.reciever(): Invalid ipv6 format: {0}", deviceIPv6));
                    }
                }
            }
        }
