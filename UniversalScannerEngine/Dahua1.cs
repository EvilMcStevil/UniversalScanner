...
{
    class Dahua1 : ScanEngine, IScanner
    {
        ...
        public ILogger<Dahua1> Logger { get; }
        public Dahua1(ILogger<Dahua1> logger, IConfig config) : base(logger, config)
        {
            this.Logger = logger;
            if (listenUdpGlobal(port) == -1)
            {
               Logger.LogWarning( String.Format("Warning: Dahua protocol v1: Failed to listen on port {0}", port));
            }
            listenUdpInterfaces();
        }
        ...
        public override void reciever(IPEndPoint from, byte[] data)
        {
            ...
            {
                Logger.LogWarning( String.Format("Warning: Dahua1.reciever(): Invalid packet size (less than section1) from {0}", from.ToString()));
                return;
            }
            ...
            {
                Logger.LogWarning( String.Format("Warning: Dahua1.reciever(): Wrong header magic recieved from {0}", from.ToString()));
                return;
            }
            ...
            {
                Logger.LogWarning( String.Format("Warning: Dahua1.reciever(): Packet has wrong size = {0} (expected was {1})", data.Length, section1Len + section2Len + section3Len));
                return;
            }
            ...
            if (deviceIPv6 != null)
            {
                ...
                {
                    Logger.LogWarning( String.Format("Warning: Dahua1.reciever(): Invalid ipv6 format: {0}", deviceIPv6));
                }
            }
