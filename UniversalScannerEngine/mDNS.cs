 ...
 public   class mDNS : ScanEngine
    {
        ...
        public ILogger/*<mDNS>*/ Logger { get; }
        public mDNS(ILogger/*<mDNS>*/ logger, IConfig config) : base(logger, config)
        {
            this.Logger = logger;
        ...
        public void scan(string queryString, mDNSType queryType)
        {
            ...
            if (globalListener.inUse)
            {
                ...
                {
                   Logger.LogError(  String.Format("Error: mDNS.scan(): Unable to send data to {0}!", endpoint.ToString()));
                }
            }
        ...
        public override void reciever(IPEndPoint from, byte[] data)
        {
            ...
            {
               Logger.LogWarning( "Warning: mDNS.reciever(): invalid packet size.");
                Logger.WriteData(LogLevel.Warning, data);
                return;
            }
            ...
            if (expectedAnwers > 0)
            {
                ...
                {
                   Logger.LogWarning( String.Format("Warning: mDNS.reciever(): packet parsing overflow at position 0x{0:X}!", position));
        ...
        private void readQueries(byte[] data, ref int position, int expectedQueries)
        {
            ...
            while (position < data.Length && expectedQueries > 0)
            {
               ...
               Logger.LogDebug(String.Format("mDNS Query '{0}', type = {1}", name, queryType));
            ...
            while (position < data.Length && answerIndex < expectedAnswers)
            {
                ...
                switch (answerType)
                {
                    case (UInt16)mDNSType.TYPE_A:
                       ...
                       Logger.LogDebug(String.Format("* mDNS answer for '{0}': IPv4 (A) = {1}", name, ip.ToString()));
                        break;
                    case (UInt16)mDNSType.TYPE_PTR:
                        string domain;
                        domain = readAnswer_PTR(data, ref position, dataLen);
                        answers[answerIndex].data.typePTR = domain;
                       Logger.LogDebug(String.Format("* mDNS answer for '{0}': Domain (PTR) = '{1}'", name, domain));
                        break;
                    case (UInt16)mDNSType.TYPE_TXT:
                        ...
                        {
                           Logger.LogDebug(String.Format("* mDNS answer for '{0}': Text (TXT) = '{1}'", name, t));
                        }
                        break;
                    case (UInt16)mDNSType.TYPE_AAAA:
                       ...
                       Logger.LogDebug(String.Format("* mDNS answer for {0}: IPv6 (AAAA) = {1}", name, ipv6.ToString()));
                        break;
                    case (UInt16)mDNSType.TYPE_SRV:
                       ...
                       Logger.LogDebug(String.Format("* mDNS answer for '{0}': Server (SRV) = '{1}:{2}'", name, srv.domain, srv.port));
                        break;
                    default:
                       Logger.LogDebug(String.Format("* mDNS answer packet type {0} not implemented, parsing of this packet aborted!", answerType));
        ...
        private IPAddress readAnswer_AAAA(byte[] data, ref int position, int dataLen)
        {
            ...
            if (dataLen != 16)
            {
               Logger.LogWarning( "Warning: readAnswer_AAAA(): Invalid address size!");
                return IPAddress.Any;
            }
        ...
        private mDNSAnswerDataSRV readAnswer_SRV(byte[] data, ref int position, int dataLen)
        {
            ...
            if (dataLen < 6)
            {
               Logger.LogWarning( "Warning: readAnswer_SRV(): packet data size error!");
