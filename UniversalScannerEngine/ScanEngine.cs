...
namespace UniversalScanner
{
    ...
    public abstract class ScanEngine : IDisposable
    {
        ...
        public ScanEngine(ILogger logger, IConfig config)
        {
            this.Config = config;
            this.Logger = logger;
        ...
        public int listenUdpGlobal(int localPort = 0)
        {
            ...
            if (localPort != 0)
            {
                ...
                {
                    Logger.LogWarning( String.Format("Warning: ScanEngine.listenUdpGlobal(): Local UDP port {0} is already in use...", localPort));
                    if (Config.portSharing)
                    {
                        Logger.LogWarning( String.Format("Warning: ScanEngine.listenUdpGlobal(): Trying to share the port {0}...", localPort));
                        reuseAddress = true;
                    }
                    else
                    {
            ...
            try
            {
               ...
               Logger.LogInformation(  String.Format("Listening on {0}...", globalListener.endPoint.ToString()), globalListener.thread.ManagedThreadId);
            }
            catch
            {
                globalListener.inUse = false;
        ...
        public void listenUdpInterfaces()
        {
            ...
            for (int i = 0; i < len; i++)
            {
                ...
                try
                {
                   ...
                   Logger.LogInformation(  String.Format("Listening on {0}...", interfacesListerner[i].endPoint.ToString()), interfacesListerner[i].thread.ManagedThreadId);
                }
                catch
                {
        ...
        public bool send(IPEndPoint endpoint)
        {
            ...
            {
                Logger.LogCritical( "Error: sendNetScan(): no opened sockets, you must call listenUdpInterfaces(), listenUdpGlobal() or listenMulticast() before.");
            ...
            {
                Logger.LogInformation(  String.Format("{0} -> {1}", multicastListener.endPoint.ToString(), endpoint.ToString()));
                Logger.WriteData(LogLevel.Debug, data);
                try
                {
            ...
            {
                Logger.LogInformation(  String.Format("{0} -> {1}", globalListener.endPoint.ToString(), endpoint.ToString()));
                Logger.WriteData(LogLevel.Debug, data);
                try
                {
            ...
            if (interfacesListerner != null)
            {
                foreach (networkBundle net in interfacesListerner)
                {
                    if (net.inUse)
                    {
                        Logger.LogInformation(  String.Format("{0} -> {1}", net.endPoint.ToString(), endpoint.ToString()));
                        Logger.WriteData(LogLevel.Debug, data);
                        try
                        {
        ...
        public bool sendNetScan(int port)
        {
            ...
            {
               Logger.LogCritical( "Error: sendNetScan(): no opened sockets, you must call listenUdpInterfaces() or listenUdpGlobal() before.");
                throw new InvalidOperationException("No opened sockets");
                //return false;
            }
            scannerPort = port;
            ...
            foreach (networkBundle net in interfacesListerner)
            {
                if (net.inUse && net.endPoint.Address.isPrivate())
                {
                    ...
                    foreach (IPAddress local in subNetAddresses)
                    {
                       ...
                       Logger.LogCritical( String.Format("{0} -> {1}", net.endPoint.ToString(), endpoint.ToString()));
                        Logger.WriteData(LogLevel.Debug, data);
        ...
        public void selfTest(string filename=null)
        {
            ...
            if (File.Exists(filename))
            {
                ...
                {
                   Logger.LogDebug(  String.Format("Error while performing self test for protocol {0}", name));
                   Logger.LogDebug(  e.ToString());
                }
            }
            else
            {
               Logger.LogDebug(  String.Format("Warning: Self test file not found for protocol {0}: '{1}' is missing", name, filename));
            }
        }
        ...
        protected int getFreeUdpPort()
        {
            ...
            if (countFree == 0)
            {
               Logger.LogCritical( "Error: UdpFreePortProvider(): No free UDP port!");
        ...
        private void unicastReciever()
        {
            ...
            {
               Logger.LogError(  String.Format("Error: ScanEngine.unicastReciever(): Unable to bind {0}!", localEP.ToString()));
            ...
            while (!closing)
            {
                ...
                try
                {
                   ...
                   Logger.LogInformation(  String.Format("{0} <- {1}", localEP.ToString(), distantEP.ToString()));
                    Logger.WriteData(LogLevel.Debug, data);
        ...
        private void multicastReciever()
        {
            ...
            {
               Logger.LogError(  String.Format("Error: multicastReciever(): Unable to enable option ReuseAddress for socket {0}!", multicastListener.endPoint.ToString()));
               Logger.LogError(  String.Format("Error: multicastReciever(): {0}", ex.ToString()));
            }

            foreach (var opt in multicastOption)
            {
               Logger.LogInformation(  String.Format("Joining group {0} on interface {1}", opt.Group.ToString(), opt.LocalAddress.ToString()));
                try
                {
                ...
                {
                   Logger.LogError(  String.Format("Error: multicastReciever(): Unable to join group {0} on interface {1}!", opt.Group.ToString(), opt.LocalAddress.ToString()));
                   Logger.LogError(  String.Format("Error: multicastReciever(): {0}", ex.ToString()));
                }
            }
            ...
            {
               Logger.LogError(  String.Format("Error: multicastReciever(): Unable to bind {0}!", multicastListener.endPoint.ToString()));
               Logger.LogError(  String.Format("Error: multicastReciever(): {0}", ex.ToString()));
                return;
            }
            ...
            while (!closing)
            {
                ...
                try
                {
                   ...
                   Logger.LogInformation(  String.Format("{0} <- {1}", multicastListener.endPoint.ToString(), distantEP.ToString()));
                    Logger.WriteData(LogLevel.Debug, data);
            ...
            foreach (var opt in multicastOption)
            {
                try
                {
                   Logger.LogInformation(  String.Format("Leaving group {0} on interface {1}", opt.Group.ToString(), opt.LocalAddress.ToString()));
