...
{
    class Hikvision : ScanEngine, IScanner
    {
        ...
        public ILogger<Hikvision> Logger { get; }
        public Hikvision(ILogger<Hikvision> logger, IConfig config) : base(logger, config)
        {
            this.Logger = logger;
        ...
        static Guid AssemblyGuid = Guid.NewGuid();
        ...
        public override void reciever(IPEndPoint from, byte[] data)
        {
            ...
            {
                ip = from.Address;
                Logger.LogWarning( String.Format("Warning: Hikvision.reciever(): Invalid ipv4 format: {0}", deviceIPv4));
            }
            ...
            if (deviceIPv6 != null)
            {
                ...
                {
                    Logger.LogWarning( String.Format("Warning: Hikvision.reciever(): Invalid ipv6 format: {0}", deviceIPv6));
                }
            }
        }
